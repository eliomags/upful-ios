...
class HoldingTests: XCTestCase {
    ...
    fileprivate func makeSUTWithTwoBuysAndUpdatedBuy() -> Holding {
        let transactions = [
            ...
            TransactionAdapter(ticker: "FB", shares: 1, tradePrice: 5)
        ]
        for transaction in transactions {
            transaction.type = TransactionType.buy.rawValue
        }
        let holding = Holding(ticker: "FB", transactions: transactions)
    ...
    fileprivate func makeSUTWithTwoBuysAndSell() -> Holding {
        ...
        let transaction3 = TransactionAdapter(ticker: "FB", shares: 1, tradePrice: 5)
